...
class ActionExtension(Extension):
    ...
    def on_stop(self, rte: RteEnv) -> None:
        logger.info("ActionExtension on_stop")
        rte.on_stop_done()

    def on_cmd(self, rte: RteEnv, cmd: Cmd) -> None:
        logger.info("ActionExtension on_cmd")
        cmd_json = cmd.to_json()
        logger.info("ActionExtension on_cmd json: " + cmd_json)
        ...
        if cmd_name == CMD_IN_FLUSH:
            if self.bedrock_llm.config.mode != 'translate':
                ...
                logger.info(f"ActionExtension on_cmd sent flush")
            else:
                logger.info(f"ActionExtension on_cmd ignore flush in '{self.bedrock_llm.config.mode}' mode.")
        else:
            logger.info(f"ActionExtension on_cmd unknown cmd: {cmd_name}")
    ...
    def on_data(self, rte: RteEnv, data: Data) -> None:
        ...
        """
        logger.info(f"ActionExtension on_data")
        ...
        thread.start()
        logger.info(f"ActionExtension on_data end")


@register_addon_as_extension("action_python")
class ActionExtensionAddon(Addon):
    def on_create_instance(self, rte: RteEnv, addon_name: str, context) -> None:
        logger.info("on_create_instance")
        rte.on_create_instance_done(ActionExtension(addon_name), context)
